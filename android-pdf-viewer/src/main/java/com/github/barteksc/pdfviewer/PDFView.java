...
public class PDFView extends RelativeLayout {
    ...
    private boolean dragScaleFlag = false;
    ...
    public PDFView(Context context, AttributeSet set) {
        ...
        dragPinchManager = new DragPinchManager(this, animationManager, dragScaleFlag);
    ...
    public void setScanTouchListener(OnScanTouchListener scanTouchListener) {
        ...
        dragPinchManager.setScanTouchListener(scanTouchListener);
    }
    ...
    public void jumpTo(int page, boolean withAnimation) {
        ...
        if (swipeVertical) {
            //计算自动切换后不居中的问题 modify by bob 2019-07-08
            offset = (offset + (getHeight() - pageSize.getHeight()) / 2);
            if (withAnimation) {
            ...
            //计算自动切换后不居中的问题 modify by bob 2019-07-08
            offset = (offset + (getWidth() - pageSize.getWidth()) / 2);
            if (withAnimation) {
    ...
    void loadComplete(PdfFile pdfFile) {
        ...
        //add by bob:新增是否禁止手势放大缩小
        if (dragScaleFlag) {
            dragPinchManager.enableScale();
        } else {
            dragPinchManager.disableScale();
        }
     ...
     * add by bob
     */
    public void disableDragScale() {
        this.dragScaleFlag = false;
    }

    /**
     * add by bob
     */
    public void enableDragScale() {
        this.dragScaleFlag = true;
    }
